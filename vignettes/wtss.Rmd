...
In what follows, we use the term "coverage" in the same sense as the definition of a "data cube", proposed from Appel and Pebesma \cite{Appel2019}: *A regular, dense Earth observation (EO) data cube is a four-dimensional array with dimensions x (longitude or easting), y (latitude or northing), and time, with the following properties:*
...
This operation requests the time series of values of a coverage attribute at a given location. Its parameters are: (a) *wtss.obj*: either a WTSS object (created by the operation wtss::WTSS as shown above) or a valid WTSS server URL; (b) *name* =           Coverage name.
