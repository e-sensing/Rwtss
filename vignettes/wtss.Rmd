...
abstract: The WTSS-R package is a front-end to the Web Time Series Service (WTSS)
  ...
  for big data analytics. A WTSS server takes as input a n Earth observation data cube, that has
...
vignette: |
  ...
  %\VignetteIndexEntry{WTSS - R interface to Web Time Series Service}
---
...
Since many time series analysis functions in R require data to be made available in the "zoo" and "ts" formats, the *wtss* package provides two convenience functions: *wtss_to_zoo* and "*wtss_to_ts*, as illustrated below.
...
# detect trends
stl <- stats::stl(ndvi_ts, s.window = 7)
# plot the result
plot(stl)
```
